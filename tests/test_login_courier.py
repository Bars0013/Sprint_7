...
class TestLoginCourier:
    ...
    @allure.description('Отправляем запрос на авторизацию в сервисе, проверяем и удаляем курьера')
    ...
    @allure.title('Проверка ошибки при авторизации курьера без заполнения обязательных поле Login/Password')
    @allure.description('Отправляем запрос на авторизацию в сервисе без обязательных полей Login/Password'
                        'и проверяем ответ')
    ...
    def test_courier_login_without_parametrs_failed(self, courier_data):
